...
int main(int argc, char** argv)
{
    ...
    // load obj model
    obj.read("debugger_50k.obj");
    //obj.printSelf();

    sphere.set(sphereRadius, 100, 100);
    point.setRadius(sphereRadius * 0.05f);
...
void draw3D()
{
    ...
    glLineWidth(2);
    glBegin(GL_LINES);
    // the line of the mouse down start point to the sphere center 
    glVertex3f(0, 0, 0);
    glVertex3f(sphereVector.x, sphereVector.y, sphereVector.z);
    // the line of the mouse down current to the sphere center 
    glVertex3f(0, 0, 0);
    ...
    //the rubber duck geometry mesh drawing
    drawObj(obj);
...
void drawObj(ObjModel& obj)
{
    ...
    {
        glDrawElements(GL_TRIANGLES, obj.getIndexCount(i), GL_UNSIGNED_INT, obj.getIndices(i));
    }
...
void mouseMotionCB(int x, int y)
{
    if (mouseLeftDown)
    {
        Vector3 v1 = trackball.getUnitVector(prevX, prevY);
...
void mousePassiveMotionCB(int x, int y)
{
    mouseX = x;
    mouseY = y;
    sphereVector = trackball.getVector(mouseX, mouseY);
}
